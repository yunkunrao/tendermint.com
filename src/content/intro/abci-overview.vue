<template><div><h1>What is TMSP?</h1>
<p>The Tendermint Socket Protocol (TMSP) allows for Byzantine Fault Tolerant replication of applications written in any programming language.</p>
<h2>Motivation</h2>
...
We do this by abstracting away the details of the application to an interface, which is implemented as a socket protocol; namely, the Tendermint Socket Protocol (TMSP).</p>
<h2>Intro to TMSP</h2>
<p><a href=https://github.com/tendermint/tendermint>Tendermint Core</a> (the &#x201C;consensus engine&#x201D;) communicates with the application via a socket protocol called <a href=https://github.com/tendermint/tmsp>TMSP</a>.</p>
...
<p>The messages are specified here: <a href=https://github.com/tendermint/tmsp#message-types>TMSP Message Types</a></p>
...
<p>There can be multiple TMSP socket connections to an application.  Tendermint Core creates three TMSP connections to the application; one for the validation of transactions when broadcasting in the mempool, one for the consensus engine to run block proposals, and one more for querying the application state.</p>
<p>It&#x2019;s probably evident that applications designers need to very carefully design their message handlers to create a blockchain that does anything useful but this architecture provides a place to start. The diagram below illustrates the flow of messages via TMSP.</p>
